...
export default async function handler(req, res) {
  ...
  // ── OpenWeather (primary, requires OPENWEATHER_API_KEY env var) ──────────
  if (key) {
    try {
      ...
      // Build 3-day forecast from 3h buckets
      ...
      // Fall through to Open-Meteo backup
  ...
  try {
    ...
    const WC={0:'☀️',1:'🌤️',2:'⛅',3:'☁️',45:'🌫️',48:'🌫️',51:'🌦️',53:'🌦️',55:'🌦️',61:'🌧️',63:'🌧️',65:'🌧️',71:'❄️',73:'❄️',75:'❄️',80:'🌦️',81:'🌦️',82:'⛈️',95:'⛈️',96:'⛈️',99:'⛈️'};
    const WD={0:'Jasno',1:'Prevaž. jasno',2:'Čiastočne oblačno',3:'Zamračené',45:'Hmla',51:'Slabé mrholenie',61:'Slabý dážď',63:'Dážď',65:'Silný dážď',71:'Slabý sneh',73:'Sneh',75:'Silný sneh',80:'Lejak',95:'Búrka'};
    const c=data.current;
    const DAYS=['Ne','Po','Ut','St','Št','Pi','So'];
    const forecast=data.daily.time.slice(1,4).map((t,i)=>{
      const dt=new Date(t+'T12:00:00');
      return{name:DAYS[dt.getDay()],icon:WC[data.daily.weather_code[i+1]]||'🌡️',hi:Math.round(data.daily.temperature_2m_max[i+1]),lo:Math.round(data.daily.temperature_2m_min[i+1])};
    });
    res.setHeader('Cache-Control','s-maxage=600,stale-while-revalidate');
    return res.json({
      _source:'open-meteo',
